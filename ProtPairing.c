...
/*
	...
	char *str = "1011010101011001110111000001000100101000100001100101111";
	int i;
	for(i=0;i<54;i++){
		if(str[i]=='1'){
			mpz_mul(Ans_Copy,Ans_Copy,A);
			mpz_fdiv_r(Ans_Copy,Ans_Copy,prime);
		}
		mpz_mul(Ans_Copy,Ans_Copy,Ans_Copy);
		mpz_fdiv_r(Ans_Copy,Ans_Copy,prime);
	}
...
void Fp4_Set1(struct Fp4 *X){
	mpz_set_str(X->tau[0],"25522886250087216",10);
	mpz_set_str(X->tau[1],"25522886250087216",10);
	mpz_set_str(X->tau[2],"25522886250087216",10);
	mpz_set_str(X->tau[3],"25522886250087216",10);
}
/*
...
void Fp4_Frob(struct Fp4 *Ans, struct Fp4 *X){
	...
	mpz_init(tmp);
	mpz_init(tmp1);
	mpz_add(tmp,tmp,X->tau[0]);
	mpz_add(tmp,tmp,X->tau[1]);
	mpz_add(tmp,tmp,X->tau[2]);
	mpz_add(tmp,tmp,X->tau[3]);
	mpz_sub(tmp,tmp1,tmp);
	mpz_fdiv_r_ui(tmp,tmp,p);
	mpz_set(Ans->tau[0],tmp);
	mpz_set(Ans->tau[1],tmp);
	mpz_set(Ans->tau[2],tmp);
	mpz_set(Ans->tau[3],tmp);
	*/
}
...
void Fp8_Mul(struct Fp8 *Ans, struct Fp8 *A, struct Fp8 *B){
	...
	Fp4_Init(&tmp2);
	Fp8_Init(&Ans_Copy);
	mpz_sub_ui(Theta.tau[0], prime, 3);
	mpz_sub_ui(Theta.tau[1], prime, 4);
	mpz_sub_ui(Theta.tau[2], prime, 4);
	mpz_sub_ui(Theta.tau[3], prime, 4);
	/*
...
void Fp24_Frob(struct Fp24 *Ans, struct Fp24 *X){
	...
	mpz_set(Ans_Copy.omega[2].theta[0].tau[0],X->omega[0].theta[0].tau[0]);
	mpz_set(Ans_Copy.omega[2].theta[0].tau[1],X->omega[0].theta[0].tau[1]);
	mpz_set(Ans_Copy.omega[2].theta[0].tau[2],X->omega[0].theta[0].tau[2]);
	mpz_set(Ans_Copy.omega[2].theta[0].tau[3],X->omega[0].theta[0].tau[3]);
	mpz_set(Ans_Copy.omega[2].theta[1].tau[0],X->omega[0].theta[1].tau[0]);
	mpz_set(Ans_Copy.omega[2].theta[1].tau[1],X->omega[0].theta[1].tau[1]);
	mpz_set(Ans_Copy.omega[2].theta[1].tau[2],X->omega[0].theta[1].tau[2]);
	mpz_set(Ans_Copy.omega[2].theta[1].tau[3],X->omega[0].theta[1].tau[3]);
	mpz_set(Ans_Copy.omega[0].theta[0].tau[0],X->omega[1].theta[0].tau[0]);
	mpz_set(Ans_Copy.omega[0].theta[0].tau[1],X->omega[1].theta[0].tau[1]);
	mpz_set(Ans_Copy.omega[0].theta[0].tau[2],X->omega[1].theta[0].tau[2]);
	mpz_set(Ans_Copy.omega[0].theta[0].tau[3],X->omega[1].theta[0].tau[3]);
	mpz_set(Ans_Copy.omega[0].theta[1].tau[0],X->omega[1].theta[1].tau[0]);
	mpz_set(Ans_Copy.omega[0].theta[1].tau[1],X->omega[1].theta[1].tau[1]);
	mpz_set(Ans_Copy.omega[0].theta[1].tau[2],X->omega[1].theta[1].tau[2]);
	mpz_set(Ans_Copy.omega[0].theta[1].tau[3],X->omega[1].theta[1].tau[3]);
	mpz_set(Ans_Copy.omega[1].theta[0].tau[0],X->omega[2].theta[0].tau[0]);
	mpz_set(Ans_Copy.omega[1].theta[0].tau[1],X->omega[2].theta[0].tau[1]);
	mpz_set(Ans_Copy.omega[1].theta[0].tau[2],X->omega[2].theta[0].tau[2]);
	mpz_set(Ans_Copy.omega[1].theta[0].tau[3],X->omega[2].theta[0].tau[3]);
	mpz_set(Ans_Copy.omega[1].theta[1].tau[0],X->omega[2].theta[1].tau[0]);
	mpz_set(Ans_Copy.omega[1].theta[1].tau[1],X->omega[2].theta[1].tau[1]);
	mpz_set(Ans_Copy.omega[1].theta[1].tau[2],X->omega[2].theta[1].tau[2]);
	mpz_set(Ans_Copy.omega[1].theta[1].tau[3],X->omega[2].theta[1].tau[3]);
...
void Fp24_Check(){
	...
	struct Fp24 A,B;
	char *str = "";
	while(str){
	Fp24_Init(&A);
	Fp24_Init(&B);
	Fp24_Rand(&A);
	Fp24_Show(&A);
	mpz_set_str(B.omega[0].theta[0].tau[0], "25522886250087216",10);
	mpz_set_str(B.omega[0].theta[0].tau[1], "25522886250087216",10);
	mpz_set_str(B.omega[0].theta[0].tau[2], "25522886250087216",10);
	mpz_set_str(B.omega[0].theta[0].tau[3], "25522886250087216",10);
	unsigned int i;
	for(i=0;i<751;i++){
		if(str[i]=='1'){
			Fp24_Mul(&B,&B,&A);
...
void EFp4_Rand(struct EFp4 *A){
	...
	mpz_sub(r4,p4,t4);
	mpz_add_ui(r4,r4,1);	//r4=p4-t4+1
	
	mpz_tdiv_q(r4,r4,order);
	mpz_tdiv_q(r4,r4,order);
	...
	do{
		...
		Fp4_Add(&tmp,&tmp,&set1);
	}while(Fp4_Legendre(&tmp)!=1);

	Fp4_Sqrt(&Ans_Copy.y,&tmp);
	EFp4_SCM(&Ans_Copy,&Ans_Copy,r4);
...
void EFp24_Rand(struct EFp24 *A){
	...
	mpz_t p4,t4,p8,t8,p24,t24,r24,tmp2;
	mpz_init(p4);
	mpz_init(t4);
	mpz_init(p8);
	mpz_init(t8);
	...
	mpz_tdiv_q(r24,r24,order);
	mpz_tdiv_q(r24,r24,order);	//r24 = r
	...
	while(Fp24_Legendre(&tmp)!=1){
		...
		Fp24_Add(&tmp,&tmp,&set1);
	}
	Fp24_Sqrt(&Ans_Copy.y,&tmp);
	//EFp24_SCM(&Ans_Copy,&Ans_Copy,r24);

	//mpz_tdiv_q(tmp2,order,r24);
	//mpz_tdiv_q(tmp2,tmp2,r24);
	//EFp24_SCM(&Ans_Copy,&Ans_Copy,tmp2);
	//EFp24_SCM(&Ans_Copy,&Ans_Copy,order);
	EFp24_SCM(&Ans_Copy,&Ans_Copy,r24);
...
int main(void){
	...
	//---------EFp Parameter set-----------
	mpz_set_str(z,"49",10);
	mpz_set_str(prime,"25522886250087217",10);
	mpz_set_str(trace,"50",10);
	mpz_set_str(order,"33232924804801",10);
